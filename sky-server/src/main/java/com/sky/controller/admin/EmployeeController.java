...
public class EmployeeController {
     ...
     * @param employeeLoginDTO 登录时传递的用户名和密码
    ...
    /**
     * 新增员工
     * @param employeeInstantDTO 新增时传递的用户名、姓名、手机号、性别、身份证号
     */
    @PostMapping
    @Operation(summary = "新增员工")
    public Result save(@RequestBody EmployeeInstantDTO employeeInstantDTO) {
        log.info("新增员工:{}", employeeInstantDTO.getUsername());

        employeeService.save(employeeInstantDTO);
    ...
    /**
     * 分页查询员工
     * @param employeeQueryDTO 查询条件（姓名、页码、每页条数）
     * @return 分页结果（不含密码）
     */
    @GetMapping("/page")
    @Operation(summary = "分页查询员工")
    public Result<PageResult<EmployeeVO>> page(EmployeeQueryDTO employeeQueryDTO) {
        log.info("分页查询员工,查询参数:{}", employeeQueryDTO);

        PageResult<EmployeeVO> pageResult = employeeService.page(employeeQueryDTO);
